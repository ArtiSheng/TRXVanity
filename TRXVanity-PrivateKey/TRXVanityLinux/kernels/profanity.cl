...
__kernel void profanity_inverse(
		__global const mp_number * restrict const pDeltaX,
		__global mp_number * restrict const pInverse) {
	const size_t id = get_global_id(0);
	...
	// Transpose the inversion batches so adjacent work-items access adjacent
	// lanes. This preserves the per-lane result while coalescing global-memory
	// traffic on wide GPU warps. Re-reading pDeltaX during the reverse pass is
	// cheaper than spilling a second 8 KiB private array on current NVIDIA GPUs.
	// buffer[i] = product(pDeltaX[id + j * stride]), for j in [0, i]
	buffer[0] = pDeltaX[id];
	size_t lane = id + stride;
	for (uint i = 1; i < PROFANITY_INVERSE_SIZE; ++i) {
		copy2 = pDeltaX[lane];
		mp_mod_mul(&buffer[i], &copy2, &buffer[i - 1]);
		lane += stride;
	}
...
static inline void profanity_iterate_with_inverse(
		__global mp_number * const pDeltaX,
		__global mp_number * const pPrevLambda,
		const size_t id,
		__private const mp_number * const pLaneInverse,
...
static inline void profanity_iterate(
		__global mp_number * const pDeltaX,
		...
		const uchar bContract,
		uint * const address) {
	mp_number laneInverse = pInverse[id];
	profanity_iterate_with_inverse(
		pDeltaX, pPrevLambda, id, &laneInverse, bContract, address);
}
