...
static_assert(sizeof(cl_ulong4) == 32, "OpenCL ulong4 ABI mismatch");
static_assert(alignof(cl_ulong4) == 32, "OpenCL ulong4 alignment mismatch");
static_assert(sizeof(mp_number) == 32, "OpenCL mp_number ABI mismatch");
static_assert(sizeof(point) == 64, "OpenCL point ABI mismatch");
static_assert(sizeof(result) == 28, "OpenCL result ABI mismatch");
static_assert(offsetof(result, foundHash) == 8, "OpenCL result layout mismatch");
...
std::filesystem::path cache_directory() {
    ...
    if (xdg_cache != nullptr && *xdg_cache != '\0') {
        result = xdg_cache;
    } else if (user_home != nullptr && *user_home != '\0') {
        result = std::filesystem::path(user_home) / ".cache";
    } else {
        result = std::filesystem::temp_directory_path();
    }
    result /= "trxvanity-linux";
    result /= "opencl";
...
bool recovered_tweak_self_test(std::string& error) {
    cl_ulong4 ordinary{{1, 2, 3, 4}};
    auto value = recovered_tweak(ordinary, 5, 6);
    if (hex_upper(value.data(), value.size())
        != "000000000000000A000000000000000300000000000000020000000000000006") {
        error = "Public GPU walk offset byte-order vector failed.";
        return false;
    }
    cl_ulong4 carry{{std::numeric_limits<std::uint64_t>::max(), 7, 8, 9}};
    value = recovered_tweak(carry, 1, 0);
    if (hex_upper(value.data(), value.size())
        != "0000000000000009000000000000000800000000000000080000000000000000") {
        error = "Public GPU walk offset carry vector failed.";
        return false;
    }
    cl_ulong4 boundary{{0, 0, 0, 0x0000FFFFFFFFFFFFULL}};
    value = recovered_tweak(boundary, 0, std::numeric_limits<std::uint32_t>::max());
    if (value[0] != 0 || value[1] != 1) {
        error = "Public GPU walk offset upper-bound vector failed.";
        return false;
    }
...
struct OpenClEngine::Impl {
    ...
    bool choose_device(std::string& error) {
        // Some minimal CUDA containers ship the Khronos loader and NVIDIA
        // implementation but omit /etc/OpenCL/vendors/nvidia.icd. The loader
        // accepts this public process-local fallback before its first query.
        if (std::getenv("OCL_ICD_FILENAMES") == nullptr
            && !std::filesystem::exists("/etc/OpenCL/vendors/nvidia.icd")) {
            (void)setenv("OCL_ICD_FILENAMES", "libnvidia-opencl.so.1", 0);
        }
    ...
    bool build_program(
        ...
        std::string& error) {
        const auto kernel_directory = executable_directory / "kernels";
        std::string keccak;
        std::string profanity;
        std::string tron;
        if (!read_file(kernel_directory / "keccak.cl", keccak, error)
            || !read_file(kernel_directory / "profanity.cl", profanity, error)
            || !read_file(kernel_directory / "tron.cl", tron, error)) {
            return false;
        }
        std::string source;
    ...
    bool recover(
        ...
        PublicKey candidate_public{};
        if (!add_public_tweak(base_public, tweak, candidate_public, error)) {
            secure_zero(tweak.data(), tweak.size());
            return false;
        }
        std::array<std::uint8_t, 32> diagnostic_hash{};
        if (!tron_address_from_public_key(
                candidate_public, address, diagnostic_hash, error)) {
            secure_zero(tweak.data(), tweak.size());
            return false;
        }
        if (std::memcmp(
        ...
        if (!matches_suffix(address, plan.suffix)) {
            secure_zero(tweak.data(), tweak.size());
            error = "GPU matcher false positive for independently verified address "
                + address + "; no search offset was released.";
            return false;
        }
        tweak_hex = hex_upper(tweak.data(), tweak.size());
        secure_zero(tweak.data(), tweak.size());
        return true;
    }
...
bool OpenClEngine::initialize(
    ...
    const ProtocolEmitter& emit,
    std::string& error) {
    if (!recovered_tweak_self_test(error)) {
        return false;
    }
    if (!validate_public_key(base_public, error)) {
        return false;
    }
    impl_->base_public = base_public;
    impl_->round = 0;
    ...
    if (!impl_->build_program(executable_directory, emit, error)) {
        return false;
    }
    ...
    if (!random_public_bytes(&impl_->seed, sizeof(impl_->seed), error)) {
        error = "Unable to create the public GPU walk offset: " + error;
        return false;
    }
    ...
    if (!impl_->set_static_arguments(impl_->base_public, error)) {
        return false;
    }
...
bool OpenClEngine::search(
    ...
    while (!stop_requested.load(std::memory_order_relaxed)) {
        if (impl_->round > std::numeric_limits<std::uint64_t>::max() - 2) {
            error = "The public GPU walk counter overflowed.";
            return false;
        }
...
bool OpenClEngine::forced_suffix_self_test(
    const ProtocolEmitter& emit,
    std::string& error) {
    ...
    for (std::size_t length = 1; length <= 10; ++length) {
        ...
        PublicKey candidate_public{};
        if (!add_public_tweak(impl_->base_public, tweak, candidate_public, error)) {
            secure_zero(tweak.data(), tweak.size());
            return false;
        }
        ...
        if (!tron_address_from_public_key(
                candidate_public, expected_address, public_hash, error)) {
            secure_zero(tweak.data(), tweak.size());
            return false;
        }
        secure_zero(tweak.data(), tweak.size());
        ...
        const bool valid = outcome.found
            && matches_suffix(outcome.address, suffix)
            && outcome.tweak_hex.size() == 64;
        if (!valid) {
            error = "The deterministic split-key GPU self-test missed its forced candidate.";
            return false;
        }
        emit("SELFTEST_SUFFIX\t" + std::to_string(length) + "\t"
