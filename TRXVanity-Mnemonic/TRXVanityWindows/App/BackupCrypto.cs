...
namespace TRXVanity.WindowsApp
{
    ...
    {
        private static readonly byte[] Magic = Encoding.ASCII.GetBytes("TRXMNEMO");
        private static readonly byte[] AuthenticationLabel =
            Encoding.ASCII.GetBytes("TRXVanity mnemonic backup authentication");
            ...
            string suffix)
        {
            ...
            AppendJsonProperty(result, "mnemonic", mnemonic, true);
            AppendJsonProperty(result, "derivationPath", derivationPath, true);
            AppendJsonProperty(
                result,
                "createdUtc",
