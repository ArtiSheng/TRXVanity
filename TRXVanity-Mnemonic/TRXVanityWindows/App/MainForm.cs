...
namespace TRXVanity.WindowsApp
{
    internal sealed class MainForm : Form
    {
        ...
        private ComboBox computeProfileCombo;
        private Label computeProfileHint;
        ...
        private TextBox mnemonicText;
        private Button revealButton;
        private Button copyAddressButton;
        private Button copyMnemonicButton;
        ...
        private void BuildInterface()
        {
            ...
            Label footer = MakeLabel(
                "CUDA + CPU 并行 BIP39 · TronLink 标准路径 · 独立复验 · 本机历史使用 DPAPI",
                9F,
                FontStyle.Regular,
                Muted);
        ...
        private Control BuildHeader()
        {
            ...
            Label subtitle = MakeLabel("TronLink 可导入助记词 · GPU 高速离线靓号生成器", 9F, FontStyle.Regular, Muted);
            ...
            pills.Controls.Add(MakePill("CUDA BIP39", Accent));
        ...
        private Control BuildConfigurationCard()
        {
            ...
            TableLayoutPanel content = NewCardLayout(8);
            ...
            content.RowStyles.Add(new RowStyle(SizeType.Absolute, Scaled(82F)));
            content.RowStyles.Add(new RowStyle(SizeType.Absolute, Scaled(76F)));
            content.RowStyles.Add(new RowStyle(SizeType.Absolute, Scaled(42F)));
            ...
            content.Controls.Add(BuildComputeProfileRow(), 0, 3);
            ...
            content.Controls.Add(difficultyLabel, 0, 4);
            ...
            content.Controls.Add(warning, 0, 5);

            startButton = NewButton("GPU 正在初始化…", Accent, Color.White);
            startButton.Enabled = false;
            startButton.Dock = DockStyle.Fill;
            startButton.Click += StartButtonClick;
            content.Controls.Add(startButton, 0, 6);
            ...
            content.Controls.Add(shortcut, 0, 7);
        ...
        private Control BuildComputeProfileRow()
        {
            TableLayoutPanel row = new TableLayoutPanel();
            row.Dock = DockStyle.Fill;
            row.ColumnCount = 2;
            row.RowCount = 2;
            row.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute, Scaled(92F)));
            row.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 100F));
            row.RowStyles.Add(new RowStyle(SizeType.Absolute, Scaled(36F)));
            row.RowStyles.Add(new RowStyle(SizeType.Percent, 100F));

            Label title = MakeLabel("计算方案", 9F, FontStyle.Bold, Ink);
            title.TextAlign = ContentAlignment.MiddleLeft;
            row.Controls.Add(title, 0, 0);

            computeProfileCombo = new ComboBox();
            computeProfileCombo.Dock = DockStyle.Fill;
            computeProfileCombo.DropDownStyle = ComboBoxStyle.DropDownList;
            computeProfileCombo.FlatStyle = FlatStyle.Flat;
            computeProfileCombo.Font = new Font("Microsoft YaHei UI", 9F, FontStyle.Regular);
            computeProfileCombo.BackColor = Color.White;
            computeProfileCombo.ForeColor = Ink;
            computeProfileCombo.Items.Add(new ComputeProfileOption(
                "rtx5070",
                "RTX 5070 方案",
                "锁定 RTX 5070 的分阶段 CUDA 参数，并自动调节满速批量。"));
            computeProfileCombo.Items.Add(new ComputeProfileOption(
                "rtx4090",
                "RTX 4090 方案",
                "使用实测最快的 256/384 线程块、双流交错与曲线批量求逆。"));
            computeProfileCombo.Items.Add(new ComputeProfileOption(
                "smart",
                "智能最高速（任意 RTX）",
                "检测 RTX 架构、计算单元、显存与内核占用率，自动选择最高速参数。"));
            computeProfileCombo.SelectedIndex = 2;
            computeProfileCombo.SelectedIndexChanged += ComputeProfileChanged;
            row.Controls.Add(computeProfileCombo, 1, 0);

            computeProfileHint = MakeLabel(string.Empty, 8F, FontStyle.Regular, Muted);
            computeProfileHint.TextAlign = ContentAlignment.TopLeft;
            computeProfileHint.Padding = ScaledPadding(0, 2, 0, 0);
            row.Controls.Add(computeProfileHint, 0, 1);
            row.SetColumnSpan(computeProfileHint, 2);
            UpdateComputeProfileHint(false, string.Empty, string.Empty);
            return row;
        }
        ...
        private Control BuildResultCard()
        {
            ...
            mnemonicText = NewResultTextBox();
            mnemonicText.UseSystemPasswordChar = true;
            revealButton = NewSmallButton("显示");
            revealButton.Enabled = false;
            revealButton.Click += RevealButtonClick;
            copyMnemonicButton = NewSmallButton("复制助记词");
            copyMnemonicButton.Enabled = false;
            copyMnemonicButton.Click += delegate { CopySecure(mnemonicText.Text, true); };
            content.Controls.Add(BuildResultRow(
                "12 词 BIP39 助记词（TronLink 路径 m/44'/195'/0'/0/0）",
                mnemonicText,
                copyMnemonicButton,
                revealButton), 0, 2);
            card.Controls.Add(content);
        ...
        private Control BuildHistoryCard()
        {
            ...
            buttons.Controls.Add(HistoryButton("复制助记词", CopyHistoryMnemonic));
        ...
        private ComputeProfileOption SelectedComputeProfile()
        {
            ComputeProfileOption option = computeProfileCombo == null
                ? null
                : computeProfileCombo.SelectedItem as ComputeProfileOption;
            return option ?? new ComputeProfileOption(
                "smart",
                "智能最高速（任意 RTX）",
                "检测 RTX 架构、计算单元、显存与内核占用率，自动选择最高速参数。");
        }

        private static string ComputeProfileDisplayName(string id)
        {
            switch (id)
            {
                case "rtx5070": return "RTX 5070 方案";
                case "rtx4090": return "RTX 4090 方案";
                default: return "智能最高速（任意 RTX）";
            }
        }

        private void UpdateComputeProfileHint(bool active, string activeId, string details)
        {
            if (computeProfileHint == null)
            {
                return;
            }
            if (active)
            {
                computeProfileHint.Text = "已启用：" + ComputeProfileDisplayName(activeId)
                    + (string.IsNullOrEmpty(details) ? string.Empty : " · " + details);
                computeProfileHint.ForeColor = Success;
                return;
            }
            ComputeProfileOption selected = SelectedComputeProfile();
            computeProfileHint.Text = selected.Description;
            computeProfileHint.ForeColor = Muted;
        }

        private void ComputeProfileChanged(object sender, EventArgs args)
        {
            UpdateComputeProfileHint(false, string.Empty, string.Empty);
            if (!formShown || closingApplication || running)
            {
                return;
            }
            engineStatusLabel.Text = "正在切换计算方案并重新初始化 GPU…";
            engineStatusLabel.ForeColor = Muted;
            engineProgress.Style = ProgressBarStyle.Marquee;
            StopEngineProcess(true, 700);
            StartEngine();
        }
        ...
        private void StartEngine()
        {
            ...
            engineReady = false;
            computeProfileCombo.Enabled = false;
            startButton.Enabled = false;
            startButton.Text = "GPU 正在初始化…";
            engineProgress.Style = ProgressBarStyle.Continuous;
            engineProgress.Value = 0;
            lock (engineErrorLog)
            {
                engineErrorLog.Clear();
            }
            ...
            try
            {
                ...
                info.Arguments = "--server --profile " + SelectedComputeProfile().Id;
        ...
        private void StopEngineProcess(bool graceful, int waitMilliseconds)
        {
            Process process = engineProcess;
            StreamWriter input = engineInput;
            engineProcess = null;
            engineInput = null;
            engineReady = false;
            if (process == null)
            ...
            process.OutputDataReceived -= EngineOutputReceived;
            process.ErrorDataReceived -= EngineErrorReceived;
            process.Exited -= EngineExited;
            try
            {
                if (graceful && input != null && !process.HasExited)
                {
        ...
        private void EngineOutputReceived(object sender, DataReceivedEventArgs args)
        {
            ...
            if (!object.ReferenceEquals(source, engineProcess)
                || string.IsNullOrEmpty(args.Data) || IsDisposed || !IsHandleCreated)
            {
                return;
            }
            try
        ...
        {
            if (object.ReferenceEquals(sender, engineProcess) && !string.IsNullOrEmpty(args.Data))
            {
                lock (engineErrorLog)
                {
        ...
        private void EngineExited(object sender, EventArgs args)
        {
            ...
            try
            {
                ...
                {
                    if (closingApplication || !object.ReferenceEquals(source, engineProcess))
                    {
                        return;
                    }
                    ...
                    else
                    {
                        ...
                        startButton.Enabled = false;
                        computeProfileCombo.Enabled = true;
        ...
        private void HandleEngineLine(string line)
        {
            ...
            switch (fields[0])
            {
                ...
                case "READY":
                    engineReady = true;
                    gpuLabel.Text = "GPU+CPU 已连接";
                    engineStatusLabel.Text = "GPU+CPU 已就绪；输出为 TronLink 可导入的标准 12 词助记词";
                    engineStatusLabel.ForeColor = Success;
                    engineProgress.Value = 100;
                    startButton.Text = "开始并行搜索";
                    startButton.Enabled = HasValidConditions();
                    computeProfileCombo.Enabled = true;
                    string profileId = fields.Length >= 4
                        ? fields[3]
                        : SelectedComputeProfile().Id;
                    string profileDetails = string.Empty;
                    ulong readyLanes;
                    if (fields.Length >= 3
                        && ulong.TryParse(fields[2], NumberStyles.Integer, CultureInfo.InvariantCulture, out readyLanes))
                    {
                        profileDetails = readyLanes.ToString("N0", CultureInfo.CurrentCulture) + " 最大批容量";
                    }
                    if (fields.Length >= 9)
                    {
                        profileDetails += (profileDetails.Length == 0 ? string.Empty : " · ")
                            + (fields[8] == "cuda-bip39+openssl-cpu"
                                ? "CUDA + OpenSSL CPU"
                                : fields[8] == "cuda-bip39"
                                    ? "CUDA BIP39/PBKDF2"
                                : fields[8]);
                    ...
                    uint cpuWorkers;
                    if (fields.Length >= 5
                        && uint.TryParse(fields[4], NumberStyles.Integer, CultureInfo.InvariantCulture, out cpuWorkers)
                        && cpuWorkers > 0)
                    {
                        profileDetails += (profileDetails.Length == 0 ? string.Empty : " · ")
                            + cpuWorkers.ToString("N0", CultureInfo.CurrentCulture) + " CPU 线程";
                    }
                    UpdateComputeProfileHint(true, profileId, profileDetails);
                    SetHeartbeatState("ready", engineStatusLabel.Text);
                    SendHeartbeat(string.Empty, string.Empty, string.Empty);
                    break;

                case "SEARCHING":
                    engineStatusLabel.Text = "GPU+CPU 并行满负载搜索中…";
                    engineStatusLabel.ForeColor = Accent;
                    engineProgress.Style = ProgressBarStyle.Marquee;
        ...
        private void HandleResult(string[] fields)
        {
            ...
            string resultSource = fields.Length >= 6 ? fields[5] : string.Empty;
            string sourceLabel = resultSource == "cpu" ? "CPU" : "GPU";
            SetRunning(false);
            addressText.Text = fields[1];
            mnemonicText.Text = fields[2];
            mnemonicText.UseSystemPasswordChar = true;
            revealButton.Text = "显示";
            revealButton.Enabled = true;
            copyAddressButton.Enabled = true;
            copyMnemonicButton.Enabled = true;
            ...
            engineStatusLabel.Text = sourceLabel + " 命中并通过独立复验，正在加密保存…";
            engineStatusLabel.ForeColor = Success;
            engineProgress.Style = ProgressBarStyle.Continuous;
            engineProgress.Value = 100;
            UpdateDifficulty();
            ...
            SetHeartbeatState("result", sourceLabel + " 已命中并通过独立复验");
            SendHeartbeat("result", "搜索命中", fields[1]);

            HistoryRecord record = new HistoryRecord();
            record.Id = Guid.NewGuid();
            record.Address = fields[1];
            record.Mnemonic = fields[2];
            record.DerivationPath = "m/44'/195'/0'/0/0";
            ...
            try
            {
                ...
                engineStatusLabel.Text = sourceLabel + " 命中并通过独立复验，已加密保存到本机历史";
            }
            catch (Exception exception)
            {
        ...
        private void SetRunning(bool value)
        {
            ...
            computeProfileCombo.Enabled = !value && engineReady;
        ...
        {
            mnemonicText.UseSystemPasswordChar = !mnemonicText.UseSystemPasswordChar;
            revealButton.Text = mnemonicText.UseSystemPasswordChar ? "显示" : "隐藏";
        }

        private void CopySecure(string value, bool secretValue)
        {
            ...
            try
            {
                ...
                clipboardTimer.Stop();
                clipboardValue = secretValue ? value : string.Empty;
                if (secretValue)
                {
                    clipboardTimer.Start();
                }
                engineStatusLabel.Text = secretValue
                    ? "助记词已复制，30 秒后若未被覆盖将自动清空剪贴板"
                    : "地址已复制";
                engineStatusLabel.ForeColor = secretValue ? Accent : Success;
            }
            catch (Exception exception)
            {
        ...
        private void CopyHistoryMnemonic(object sender, EventArgs args)
        {
            HistoryRecord record = SelectedRecord();
            if (record != null)
            {
                CopySecure(record.Mnemonic, true);
            }
        }
        ...
        private void ExportRecords(IList<HistoryRecord> records, string defaultName)
        {
            DialogResult warning = MessageBox.Show(
                this,
                "导出文件包含明文助记词。请仅保存到可信的离线位置，并避免云盘同步。是否继续？",
                "导出助记词",
            ...
            using (SaveFileDialog dialog = new SaveFileDialog())
            {
                ...
                content.AppendLine("TRX Vanity for Windows — OFFLINE WALLET SECRET EXPORT");
                content.AppendLine("WARNING: Anyone with the mnemonic can control its funds.");
                ...
                for (int index = 0; index < records.Count; index++)
                {
                    ...
                    content.AppendLine("Mnemonic:    " + record.Mnemonic);
                    content.AppendLine("Path:        " + record.DerivationPath);
                    content.AppendLine(new string('-', 76));
                }
        ...
        {
            if (value == "Loading the BIP39 English dictionary")
            {
                return "正在加载标准 BIP39 英文词表…";
            }
            if (value == "Loading the OpenSSL CPU PBKDF2 accelerator")
            {
                return "正在加载 OpenSSL CPU PBKDF2 加速器…";
            }
            if (value == "Initializing the CUDA BIP39 kernel")
            {
                return "正在初始化 CUDA 助记词 PBKDF2 内核…";
            }
            if (value == "Cross-checking CUDA against the TronLink derivation vector")
            {
                return "正在用 TronLink 标准路径交叉校验 GPU 与 CPU…";
            }
            if (value == "Full CUDA mnemonic engine ready")
            {
                return "全 CUDA 助记词引擎已就绪";
            }
            if (value == "CUDA + CPU mnemonic engine ready")
            {
                return "CUDA + CPU 并行助记词引擎已就绪";
            }
            return value;
        }
    ...
    internal sealed class ComputeProfileOption
    {
        public ComputeProfileOption(string id, string name, string description)
        {
            Id = id;
            Name = name;
