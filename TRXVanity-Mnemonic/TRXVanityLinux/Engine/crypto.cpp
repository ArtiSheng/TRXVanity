    ...
    SecpContext() {
        ...
        if (value != nullptr) {
            ...
            if (RAND_priv_bytes_ex(
                    nullptr, seed.data(), seed.size(), 0) == 1) {
...
bool secure_random(void* data, std::size_t size, std::string& error) {
    if (size == 0) {
        error.clear();
        return true;
    }
    if (data == nullptr
        || RAND_priv_bytes_ex(
            ...
            size,
            0) != 1) {
        error = "OpenSSL secure random generation failed.";
        return false;
    }
    error.clear();
    return true;
...
bool compressed_public_key(
    ...
    if (secp256k1_ec_pubkey_serialize(
        ...
        || output_size != output.size()) {
        error = "Unable to serialize the compressed secp256k1 public key.";
        return false;
    }
    return true;
}
...
bool add_tweak(
    ...
    if (secp256k1_ec_seckey_tweak_add(ctx, output.data(), tweak.data()) != 1) {
        ...
        error = "The BIP32 child scalar is outside the secp256k1 group.";
        return false;
    }
    return true;
}
...
std::array<std::uint8_t, 32> keccak256(const std::uint8_t* data, std::size_t size) {
    ...
    while (size - cursor >= rate) {
        for (std::size_t lane = 0; lane < rate_lanes; ++lane) {
            std::uint64_t word = 0;
            // The supported Linux host is x86_64, so native words are little
...
bool sha256(
    ...
    if (EVP_Digest(
            ...
            nullptr) != 1
        || output_size != output.size()) {
        error = "OpenSSL SHA-256 hashing failed.";
        return false;
    }
    error.clear();
    return true;
