...
public:
    ...
    bool start(unsigned worker_count, std::string& error) {
        ...
        if (!secure_random(base_.data(), base_.size(), error)) {
            error = "Secure random generation failed while creating CPU BIP39 entropy.";
            return false;
        }
        try {
...
#if defined(__linux__)
        // Linux tracks niceness per task/thread. Lower CPU fallback workers so
        // the host thread can promptly feed and drain the two CUDA streams.
        (void)::setpriority(
            PRIO_PROCESS,
...
bool MnemonicEngine::initialize(
    ...
    const auto wordlist = executable_directory / "bip39-english.txt";
    ...
    // Exercise the exact 58^8 checksum prefilter with a known full CUDA
    // derivation. The eight-candidate check below retains a ten-digit suffix
    // and therefore continues to cover the generic modulus path.
    if (!MatchPlan::create(
            {},
            vector_address.substr(vector_address.size() - 8),
            vector_plan,
            error)
    ...
    // Exercise eight consecutive addresses as eight independent positive
    // vectors.  Each launch targets one candidate's unique ten-digit suffix,
    // so every grouped lane must derive the right address and publish the
    // matching master key rather than merely avoiding another lane's suffix.
    constexpr std::uint32_t group_candidate_count = 8;
    std::array<std::string, group_candidate_count> group_addresses;
    std::array<std::string, group_candidate_count> group_suffixes;
    std::string verified_group_mnemonic;
    const auto clear_test_secret = [](std::string& value) noexcept {
        if (!value.empty()) secure_zero(value.data(), value.size());
        value.clear();
    };
    const auto clear_group_test = [&]() noexcept {
        clear_test_secret(verified_group_mnemonic);
        for (auto& value : group_suffixes) clear_test_secret(value);
        for (auto& value : group_addresses) clear_test_secret(value);
        cuda_mnemonic_clear_host(group_candidate_count);
    };

    for (std::uint32_t candidate = 0;
         candidate < group_candidate_count;
         ++candidate) {
        auto candidate_entropy = entropy_at(zero, candidate);
        std::string candidate_mnemonic;
        ...
        if (!derive_mnemonic_candidate(
                ...
                candidate_address,
                error)) {
            clear_test_secret(candidate_mnemonic);
            clear_test_secret(candidate_address);
            secure_zero(candidate_entropy.data(), candidate_entropy.size());
            clear_group_test();
            cuda_mnemonic_shutdown();
            return false;
        }
        clear_test_secret(candidate_mnemonic);
        secure_zero(candidate_entropy.data(), candidate_entropy.size());
        group_suffixes[candidate] = candidate_address.substr(
            candidate_address.size() - 10);
        for (std::uint32_t previous = 0; previous < candidate; ++previous) {
            if (group_suffixes[previous] == group_suffixes[candidate]) {
                clear_test_secret(candidate_address);
                error = "The eight-candidate CUDA self-test suffix is not unique.";
                clear_group_test();
                cuda_mnemonic_shutdown();
                return false;
            }
        }
        group_addresses[candidate] = std::move(candidate_address);
    }

    for (std::uint32_t target = 0; target < group_candidate_count; ++target) {
        MatchPlan group_plan;
        if (!MatchPlan::create(
                {},
                group_suffixes[target],
                group_plan,
                error)
            || !cuda_mnemonic_launch_search_batch(
                0,
        ...
        if (!cuda_mnemonic_wait_search_batch(
            ...
            if (error.empty()) {
                error = "The eight-candidate CUDA address cross-check failed.";
            }
            clear_group_test();
            cuda_mnemonic_shutdown();
            return false;
        ...
        auto group_entropy = entropy_at(zero, target);
        if (!verify_mnemonic_candidate(
                group_entropy,
                impl_->words,
                group_master,
                group_addresses[target],
                verified_group_mnemonic,
                error)) {
            secure_zero(group_entropy.data(), group_entropy.size());
            clear_group_test();
            cuda_mnemonic_shutdown();
            return false;
        }
...
bool MnemonicEngine::search(
    ...
    if (!secure_random(random_base.data(), random_base.size(), error)) {
        error = "Secure random generation failed while creating BIP39 entropy.";
        return false;
    }
    ...
    while (slots[0].active || slots[1].active) {
        ...
        if (!stop_requested.load(std::memory_order_relaxed)) {
            ...
            if (!slots[current_slot].active
                ...
                if (!secure_random(
                        random_base.data(), random_base.size(), error)) {
                    secure_zero(random_base.data(), random_base.size());
                    error = "Secure random generation failed while rotating BIP39 entropy.";
                    return false;
                }
                next_first = 0;
                if (!launch_slot(current_slot)) {
...
unsigned MnemonicEngine::cpu_budget() const noexcept {
    return impl_->cpu_budget.effective_cpus;
}

const std::string& MnemonicEngine::cpu_budget_source() const noexcept {
    return impl_->cpu_budget.source;
}
