...
#ifndef TRXVANITY_ADDRESS_CANDIDATES_PER_THREAD
#define TRXVANITY_ADDRESS_CANDIDATES_PER_THREAD 4
#endif

#ifndef TRXVANITY_SECP_WINDOW_BITS
#define TRXVANITY_SECP_WINDOW_BITS 16
#endif

#ifndef TRXVANITY_MASTER_MIN_BLOCKS_PER_SM
#define TRXVANITY_MASTER_MIN_BLOCKS_PER_SM 0
#endif

#ifndef TRXVANITY_MASTER_LOW_SPILL_THREADS
#define TRXVANITY_MASTER_LOW_SPILL_THREADS 0
#endif

#ifndef TRXVANITY_PBKDF2_DIRECT_WORDS
#define TRXVANITY_PBKDF2_DIRECT_WORDS 0
#endif

static_assert(
    TRXVANITY_ADDRESS_CANDIDATES_PER_THREAD == 1
        || TRXVANITY_ADDRESS_CANDIDATES_PER_THREAD == 2
        || TRXVANITY_ADDRESS_CANDIDATES_PER_THREAD == 4
        || TRXVANITY_ADDRESS_CANDIDATES_PER_THREAD == 8,
    "TRXVANITY_ADDRESS_CANDIDATES_PER_THREAD must be 1, 2, 4, or 8");
static_assert(
    TRXVANITY_SECP_WINDOW_BITS >= 8 && TRXVANITY_SECP_WINDOW_BITS <= 20,
    "TRXVANITY_SECP_WINDOW_BITS must be between 8 and 20");
static_assert(
    TRXVANITY_MASTER_MIN_BLOCKS_PER_SM == 0
        || TRXVANITY_MASTER_MIN_BLOCKS_PER_SM == 2,
    "TRXVANITY_MASTER_MIN_BLOCKS_PER_SM must be 0 or 2");
static_assert(
    TRXVANITY_MASTER_LOW_SPILL_THREADS == 0
        || TRXVANITY_MASTER_LOW_SPILL_THREADS == 96
        || TRXVANITY_MASTER_LOW_SPILL_THREADS == 128,
    "TRXVANITY_MASTER_LOW_SPILL_THREADS must be 0, 96, or 128");
static_assert(
    TRXVANITY_MASTER_LOW_SPILL_THREADS == 0
        || TRXVANITY_MASTER_MIN_BLOCKS_PER_SM == 0,
    "The two master-kernel launch-bounds experiments are mutually exclusive");
static_assert(
    TRXVANITY_PBKDF2_DIRECT_WORDS == 0
        || TRXVANITY_PBKDF2_DIRECT_WORDS == 1,
    "TRXVANITY_PBKDF2_DIRECT_WORDS must be 0 or 1");

#if TRXVANITY_MASTER_LOW_SPILL_THREADS == 96
#define TRXVANITY_MASTER_LAUNCH_BOUNDS __launch_bounds__(96, 4)
#elif TRXVANITY_MASTER_LOW_SPILL_THREADS == 128
#define TRXVANITY_MASTER_LAUNCH_BOUNDS __launch_bounds__(128, 3)
#elif TRXVANITY_MASTER_MIN_BLOCKS_PER_SM == 2
#define TRXVANITY_MASTER_LAUNCH_BOUNDS __launch_bounds__(256, 2)
#else
#define TRXVANITY_MASTER_LAUNCH_BOUNDS __launch_bounds__(256)
#endif
...
constexpr unsigned kAddressCandidatesPerThread =
...
unsigned g_secp_window_bits = TRXVANITY_SECP_WINDOW_BITS;
...
#if TRXVANITY_PBKDF2_DIRECT_WORDS == 1
// Experimental BIP39 specialization of the vendored PBKDF2 hot loop.  The
// baseline API formats its 512-bit result as big-endian bytes at the final
// destination, after which this kernel immediately parses those bytes back to
// the same eight SHA-512 words for BIP32.  Returning the words directly makes
// that handoff explicit, so SASS/resource A/B results can show whether nvlink
// LTO already removes the intermediate global traffic.  The production path
// remains byte-for-byte unchanged while the compile option is zero.
__device__ __forceinline__ void pbkdf2_bip39_sha512_direct_words(
    const std::uint8_t* phrase,
    std::size_t phrase_length,
    std::uint64_t result[8]) {
    pbkdf2_sha512_precomp_t context;
    pbkdf2_sha512_init_precomp(phrase, phrase_length, &context);
    pbkdf2_u1_sha512_words(
        &context,
        1,
        kMnemonicSalt,
    ...
    std::uint64_t u_block[16];
#pragma unroll
    for (unsigned index = 0; index < 8; ++index) {
        u_block[index] = result[index];
    }
    u_block[8] = 0x8000000000000000ULL;
#pragma unroll
    for (unsigned index = 9; index < 15; ++index) u_block[index] = 0;
    u_block[15] = 0x0000000000000600ULL;

    for (std::uint64_t iteration = 1; iteration < 2048; ++iteration) {
        sha512_raw_transform(context.inner_H, u_block, u_block);
        sha512_raw_transform(context.outer_H, u_block, u_block);
        sha512_xor(result, u_block);
    }
}
#endif

__global__ TRXVANITY_MASTER_LAUNCH_BOUNDS void derive_mnemonic_master_keys_fused(
...
#pragma unroll
    ...
    pbkdf2_bip39_sha512_direct_words(
...
void cuda_mnemonic_clear_slot(
    ...
    if (slot.pinned_masters != nullptr) {
        secure_zero(slot.pinned_masters, 64);
    }
